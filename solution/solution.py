...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        ap = sum(precisions_at_k) / running_rel
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            f"You are an AI judge evaluating a model response.\n"
            f"Question: {question}\n"
            f"Answer: {answer}\n\n"
            f"Rubric:\n{rubric_str}\n\n"
            f"Score each criterion on a scale of 0.0 to 1.0. Output valid JSON."
        )
        ...
        try:
            ...
            if isinstance(parsed, dict):
                parsed_scores = parsed.get("scores", parsed) if isinstance(parsed.get("scores"), dict) else parsed
                for k in rubric:
                    if k in parsed_scores:
                        scores[k] = float(parsed_scores[k])
                    else:
                        scores[k] = 0.5
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        entry_avgs: list[float] = []

        for item in scores_batch:
            scores_dict = item.get("scores", item) if isinstance(item, dict) else {}
            if isinstance(scores_dict, dict):
                vals = [float(v) for v in scores_dict.values() if isinstance(v, (int, float))]
                if vals:
                    entry_avgs.append(sum(vals) / len(vals))
        ...
        positional_bias = False
        if len(entry_avgs) > 1:
            rest_avg = sum(entry_avgs[1:]) / len(entry_avgs[1:])
            positional_bias = (entry_avgs[0] - rest_avg) > 0.2
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_avg_faithfulness = sum(r.faithfulness for r in new_results) / new_n
        new_avg_relevance = sum(r.relevance for r in new_results) / new_n
        new_avg_completeness = sum(r.completeness for r in new_results) / new_n

        baseline_avg_faithfulness = sum(r.faithfulness for r in baseline_results) / base_n
        baseline_avg_relevance = sum(r.relevance for r in baseline_results) / base_n
        baseline_avg_completeness = sum(r.completeness for r in baseline_results) / base_n
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, failure in enumerate(failures, start=1):
            ...
            if idx - 1 < len(suggestions):
                ...
                fix = "Investigate and refine pipeline"
    ...
    def generate_improvement_suggestions(
        ...
        if counts.get("hallucination", 0) > 0 or any(f.faithfulness < 0.5 for f in failures):
            ...
            suggestions.append("Enforce strict system prompt grounding to only answer from retrieved context")

        if counts.get("irrelevant", 0) > 0 or any(f.relevance < 0.5 for f in failures):
            suggestions.append("Improve prompt clarity and user query understanding to ensure relevance")
        ...
        if counts.get("off_topic", 0) > 0:
            suggestions.append("Enhance intent classification and routing to handle edge cases accurately")

        default_suggestions = [
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Add few-shot examples showing complete answers to improve completeness",
            "Implement hallucination checker to filter unsupported claims",
            "Fine-tune retrieval ranking with hybrid search and reranking",
        ]
